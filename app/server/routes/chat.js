...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      const { message, conversationHistory = [], sessionId, userContext, pageContext } = req.body;
      const userId = req.user?.id || null;

      // Link session to user if authenticated
      if (userContext?.userId) {
        // Update session with user_id
        // This will be handled by chatPersistence when we save the message
      }
      ...
      // Build enhanced context for Jean AI personalization
      let enhancedContext = '';

      if (userContext?.isAuthenticated) {
        enhancedContext += `\nCurrent user: ${userContext.userName} (${userContext.userRole})`;
      }

      if (pageContext?.path) {
        enhancedContext += `\nCurrent page: ${pageContext.path}`;
        if (pageContext.jobId) enhancedContext += ` - Viewing job ID ${pageContext.jobId}`;
        if (pageContext.employerId) enhancedContext += ` - Viewing employer ID ${pageContext.employerId}`;
      }
